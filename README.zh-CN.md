...
使用 `<Ctrl-\><Ctrl-n>` 从终端模式切换到普通模式。`Alt+[` 是更顺手的选择，且适用于所有模式（normal 模式下是 no-op，其他模式等价于 Esc）；`Ctrl+[` 不可用：它就是 Esc 字节，映射它会夺走终端内程序的 Esc。`Alt+h/j/k/l` 可以在终端模式下直接切换分屏；焦点回到终端窗口时自动恢复终端模式。普通模式下 `<ScrollWheelUp>` 和 `<ScrollWheelDown>` 可滚动终端缓冲区。终端窗口不显示行号和空白标记。
