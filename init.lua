...
-- cscope_maps.nvim
-- Uses the gtags/GTAGS database. GTAGS is generated by db_build_cmd (a custom
-- script) into the same cache directory layout as gutentags
-- (~/.local/share/nvim/tags/<project-hash-dir>), and is looked up via
-- $GTAGSROOT/$GTAGSDBPATH. gutentags only handles ctags.
require("cscope_maps").setup({
  ...
  cscope = {
    ...
    picker = "quickfix",
    db_build_cmd = {
      script = vim.fn.stdpath('data') .. '/gtags-build.sh',
      args = {},
    },
    project_rooter = {
      enable = false,
...
-- Set $GTAGSROOT/$GTAGSDBPATH per project (cache dir) and auto-build when missing
vim.api.nvim_create_autocmd({ 'BufEnter' }, {
  group = vim.api.nvim_create_augroup('GTagsBuild', { clear = true }),
  callback = function(e)
    if vim.bo[e.buf].buftype ~= '' or not vim.bo[e.buf].modifiable then
      return
    end
    if vim.fn.executable('gtags') ~= 1 then
      return
    end
    if vim.fn.expand('#' .. e.buf .. ':p') == '' then
      return
    end
    local root = vim.fs.root(e.buf, vim.g.gutentags_project_root or { '.git' })
        or vim.fn.fnamemodify(vim.fn.expand('#' .. e.buf .. ':p'), ':h')
    if not root or root == '' then
      return
    end
    local dbdir = vim.fs.normalize(vim.fn['gutentags#get_cachefile'](root, ''))
    vim.env.GTAGSROOT = root
    vim.env.GTAGSDBPATH = dbdir
    -- gtags' default parser has no Lua support; point it at the config and use
    -- native-pygments so Lua gets indexed too. GTAGSCONF is required, otherwise
    -- GTAGSLABEL is silently ignored.
    vim.env.GTAGSCONF = vim.env.GTAGSCONF or '/usr/share/gtags/gtags.conf'
    vim.env.GTAGSLABEL = vim.env.GTAGSLABEL or 'native-pygments'
    -- cscope_maps only queries after an fs_stat on the db file (the cwd-relative
    -- "GTAGS"); since our GTAGS lives in the cache dir rather than the project
    -- root, point it at the cached GTAGS via "path::root" so the check passes;
    -- actual lookup still uses GTAGSROOT/GTAGSDBPATH above.
    vim.g.cscope_maps_db_file = dbdir .. '/GTAGS::' .. root
    if vim.fn.glob(dbdir .. '/GTAGS') == '' and not vim.g.cscope_maps_statusline_indicator then
      vim.schedule(function() vim.cmd('Cscope db build') end)
    end
  end,
  desc = 'set GTAGSROOT/GTAGSDBPATH and auto-build GTAGS into cache dir',
})
